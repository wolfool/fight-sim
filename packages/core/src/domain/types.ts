...
export interface Reference {
  type: 'paper' | 'database' | 'observation' | 'expert' | 'measured' | 'estimated' | 'inferred';
  title: string;
  authors?: string[];
  year: number;
  url?: string;
...
export interface NaturalWeapon {
  type: 'bite' | 'claw' | 'horn' | 'tusk' | 'body_slam' | 'kick' | 'stomp' | 'grip' | 'strike';
  name: string;
  peakForce: number;
...
export interface NaturalArmor {
  type: 'thick_skin' | 'fat_layer' | 'bone_plate' | 'fur' | 'scales' | 'thin_skin';
  location: BodyPartId[];
  thickness: number;
...
export interface AnimalTraits {
  ...
  intelligence: number;
  sociality: number;
  huntingStyle: 'ambush' | 'chase' | 'grapple' | 'bite' | 'strike' | 'pack' | 'charge';
  preferredRange: 'close' | 'mid' | 'long';
  killMethod: 'suffocation' | 'exsanguination' | 'crushing' | 'neck_break' | 'overwhelm' | 'piercing' | 'none';
...
export interface DetailedAnimalProfile {
  ...
  physical: {
    massRange: { min: number; max: number; avg: number };
    bodyLength: number;
...
ecology: {
    ...
    socialStructure: 'solitary' | 'pair' | 'family' | 'pack' | 'troop' | 'herd' | 'pride' | 'sounders' | 'complex';
    territorySize?: number;
    homeRange?: number;
  };
  lifeHistory: {
    lifespan: { wild: number; captivity: number };
