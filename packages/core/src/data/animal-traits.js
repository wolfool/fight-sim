...
exports.ANIMAL_PRESETS = exports.HUMAN_UNTRAINED = exports.WILD_BOAR = exports.WOLF = exports.GRIZZLY = exports.BROWN_BEAR = exports.LION = exports.TIGER = exports.ORANGUTAN = exports.GORILLA = exports.CHIMPANZEE = void 0;
exports.animalToFighter = animalToFighter;
var body_segments_1 = require("./body-segments");
// ============================================
// 공통 헬퍼
// ============================================
function ref(type, title, year, opts) {
    if (opts === void 0) { opts = {}; }
    return __assign({ type: type, title: title, year: year }, opts);
}
function weapon(type, name, peakForce, source, confidence, opts) {
    if (opts === void 0) { opts = {}; }
    return __assign({ type: type, name: name, peakForce: peakForce, peakForceSource: source, peakForceConfidence: confidence, contactArea: 5, damageType: 'blunt', reach: 20, usableStates: ['STANDING', 'CLINCH', 'GROUND_TOP', 'GROUND_BOTTOM'] }, opts);
}
function armor(type, location, thickness, tensileStrength, compressiveStrength, coverage) {
    return { type: type, location: location, thickness: thickness, tensileStrength: tensileStrength, compressiveStrength: compressiveStrength, coverage: coverage };
}
// ============================================
// 10종 동물 프로파일
// ============================================
...
exports.GORILLA = {
    ...
    biomechanics: {
        ...
        acceleration: { value: 12, unit: 'm/s²', source: ref('inferred', 2020), confidence: 0.5 },
...
exports.ORANGUTAN = {
    ...
    biomechanics: {
        ...
        acceleration: { value: 8, unit: 'm/s²', source: ref('inferred', 2020), confidence: 0.5 },
...
exports.LION = {
    ...
    biomechanics: {
        ...
        jumpDistance: { value: 8, unit: 'm', source: ref('observation', 2005), confidence: 0.9 },
    },
    behavior: {
        aggression: 0.85,
        territoriality: 0.9,
...
exports.BROWN_BEAR = {
    ...
    biomechanics: {
        ...
        acceleration: { value: 15, unit: 'm/s²', source: ref('inferred', 2020), confidence: 0.5 },
...
// --- 그리즐리 (불곰 아종, 더 공격적) ---
exports.GRIZZLY = __assign(__assign({}, exports.BROWN_BEAR), { taxonomy: { class: 'Mammalia', order: 'Carnivora', family: 'Ursidae', genus: 'Ursus', species: 'arctos', subspecies: 'horribilis' }, physical: { massRange: { min: 180, max: 270, avg: 230 }, bodyLength: 200, shoulderHeight: 120, sexualDimorphism: 1.3 }, biomechanics: __assign(__assign({}, exports.BROWN_BEAR.biomechanics), { swipeForce: { value: 16000, unit: 'N', source: ref('measured', 'Grizzly bear swipe', 2002, { authors: ['Herrero'] }), confidence: 0.85 }, sprintSpeed: { value: 14, unit: 'm/s', source: ref('observation', 'Grizzly speed', 2000), confidence: 0.9 }, jumpHeight: { value: 1.8, unit: 'm', source: ref('observation', 'Grizzly vertical jump', 2005, { authors: ['Herrero'] }), confidence: 0.6 }, jumpDistance: { value: 4.5, unit: 'm', source: ref('observation', 'Grizzly broad jump', 2005, { authors: ['Herrero'] }), confidence: 0.6 } }), behavior: __assign(__assign({}, exports.BROWN_BEAR.behavior), { aggression: 0.85, territoriality: 0.9, flightiness: 0.2 }), ecology: __assign(__assign({}, exports.BROWN_BEAR.ecology), { territorySize: 600, homeRange: 400 }), references: [ref('measured', 'Grizzly bear attack patterns', 2002, { authors: ['Herrero'] })] });
// --- 늑대 ---
exports.WOLF = {
...
// 전체 프리셋 맵
// ============================================
exports.ANIMAL_PRESETS = {
    chimpanzee: exports.CHIMPANZEE,
    gorilla: exports.GORILLA,
    orangutan: exports.ORANGUTAN,
    tiger: exports.TIGER,
    lion: exports.LION,
    brown_bear: exports.BROWN_BEAR,
    grizzly: exports.GRIZZLY,
    wolf: exports.WOLF,
    wild_boar: exports.WILD_BOAR,
    human_untrained: exports.HUMAN_UNTRAINED,
};
function animalToFighter(profile, overrides) {
    if (overrides === void 0) { overrides = {}; }
    var avgMass = profile.physical.massRange.avg;
    var segments = calculateAnimalSegments(profile);
    var durability = generateAnimalDurability(profile, segments);
    var stats = deriveAnimalStats(profile);
    var mentality = deriveAnimalMentality(profile);
    return {
        id: "animal_".concat(profile.taxonomy.species, "_").concat(profile.taxonomy.subspecies || ''),
        ...
        body: {
            height: profile.physical.bodyLength,
            weight: avgMass,
            bmi: avgMass / Math.pow((profile.physical.bodyLength / 100), 2),
            skeletalMuscleMass: avgMass * 0.45,
            bodyFatMass: avgMass * 0.15,
            bodyFatPercent: 15,
...
function calculateAnimalSegments(profile) {
    // 동물은 Zatsiorsky 모델 직접 적용 불가 → 체중/체장 비례로 근사
    var avgMass = profile.physical.massRange.avg;
    var bodyLength = profile.physical.bodyLength;
    // 인간 대비 체형 비율로 조정
    var scale = avgMass / 70;
    var lengthScale = bodyLength / 175;
    var segments = [];
    for (var _i = 0, _a = Object.entries(body_segments_1.ZATSIORSKY_SEGMENTS); _i < _a.length; _i++) {
        var _b = _a[_i], partId = _b[0], params = _b[1];
        var id = partId;
        var mass = avgMass * params.massPercent * (Math.pow(scale, 0.33)); // 기하학적 스케일링
        var lengthSeg = bodyLength * params.lengthPercent;
        segments.push({
            id: id,
...
function generateAnimalDurability(profile, segments) {
    var profile_durability = {};
...
function deriveAnimalStats(profile) {
    var b = profile.biomechanics;
    var beh = profile.behavior;
    var mass = profile.physical.massRange.avg;
    // 인간 앵커(1800N, 8.5m/s, 42, 700) 대비 배율
    var strengthRatio = (b.strikeForce || b.biteForce || 1800) / 1800;
    var speedRatio = b.sprintSpeed / 7.5;
    var enduranceRatio = 1.5; // 동물은 기본적으로 지구력 높음
    return {
        ...
        durability: Math.min(100, Math.round(50 * Math.pow((mass / 70), 0.33) * (1 + beh.territoriality * 0.5))),
...
function deriveAnimalMentality(profile) {
    var beh = profile.behavior;
    return {
...
//# sourceMappingURL=animal-traits.js.map