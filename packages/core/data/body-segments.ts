...
// 17개 세그먼트 파라미터 (Zatsiorsky 남성 데이터)
...
export const GROIN_PARAMS: SegmentParams = {
  massPercent: 0.010,
  lengthPercent: 0.040,
  comPosition: 0.50,
  radiusGyration: 0.35,
...
export const SEGMENT_MUSCLE_DISTRIBUTION: Partial<Record<BodyPartId, number>> = {
  head: 0.01,
  neck: 0.015,
  torso_front: 0.18,
  torso_back: 0.20,
...
export const SEGMENT_FAT_DISTRIBUTION: Partial<Record<BodyPartId, number>> = {
  head: 0.02,
  neck: 0.02,
  torso_front: 0.25,
  torso_back: 0.15,
...
export function calculateBodySegments(input: SegmentCalcInput): BodySegment[] {
  ...
  segments.push({
    id: 'groin',
    mass: Math.round(weight * GROIN_PARAMS.massPercent * adjustment.massFactor * 1000) / 1000,
    length: Math.round(height * GROIN_PARAMS.lengthPercent * adjustment.lengthFactor * 10) / 10,
    crossSection: Math.round((weight * GROIN_PARAMS.massPercent * 1000) / (height * GROIN_PARAMS.lengthPercent * avgDensity) * 100) / 100,
    boneDensity,
    muscleThickness: 1.5,
    fatThickness: 2.0,
  });
